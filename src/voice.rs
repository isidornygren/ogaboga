use crate::{
   ...
   wave_generator::{WaveBox, WaveForm, WaveGenerator},
};
...
impl VoiceHandler {
   ...
   pub fn set_freq(&mut self, freq: f32) { self.wave_gen.set_freq(freq); }

   pub fn set_amp(&mut self, amp: f32) { self.amp = amp.min(1.0).max(0.0); }

   pub fn set_waveform(&mut self, waveform: WaveBox) { self.wave_gen.set_waveform(waveform); }
   ...
   pub fn start(&mut self) { self.pulse_modulator.start(); }

   pub fn stop(&mut self) { self.pulse_modulator.stop(); }
}
